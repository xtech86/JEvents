...
zip  -r --exclude=*.svn* ../installer.zip installer
cd ../
cd libraries
...
cd ../
zip  -r --exclude=*.svn* "jevents35_$VERSION.zip" language install.php pkg_jevents.xml com_jevents.zip finder.zip search.zip gwejson.zip installer.zip googl.zip jevtypeahead.zip jevmodal.zip jevents.zip  mod_jevents_cal.zip mod_jevents_custom.zip mod_jevents_filter.zip   mod_jevents_latest.zip mod_jevents_legend.zip mod_jevents_switchview.zip

find . \! -name "createpackage.sh" \! -name "jevents35_$VERSION.zip" | xargs rm -rf
